...
| `skill_activated` | `VARCHAR(255)` | | 激活的技能名称 |
| `thinking_content` | `TEXT` | | 思考过程内容 |
...
- `message_type`: 消息类型，user表示用户发送的消息，assistant表示智能体回复的消息，system表示系统消息
- `send_time`: 消息发送时间，用于排序和查询
- `status`: 消息状态，sending（发送中）、sent（已发送）、delivered（已送达）、read（已读）、failed（失败）
- `message_length`: 消息长度，用于统计和优化
- `has_attachment`: 是否包含附件，布尔值
- `attachment_path`: 附件路径，如果消息包含附件
- `skill_activated`: 激活的技能名称，当智能体调用技能时记录
- `thinking_content`: 思考过程内容，记录智能体的推理过程
- `created_at`: 记录创建时间
- `updated_at`: 记录更新时间
...
-- 激活技能索引（用于查询哪些消息调用了技能）
CREATE INDEX IF NOT EXISTS idx_chat_messages_skill_activated ON chat_messages(skill_activated);
```
...
- `idx_chat_messages_skill_activated`: 优化按激活技能查询消息的性能
...
## 8. SQLite + IndexedDB 混合存储

### 8.1 混合存储概述

SQLite + IndexedDB 混合存储是 AutoMate 项目的核心数据存储策略，采用分层缓存架构实现高效的数据读写：

- **SQLite（主存储/冷数据）**：所有历史数据永久存储，提供持久化保障
- **IndexedDB（热缓存）**：仅存储最近 3 天的热数据，提供快速读取能力

### 8.2 存储策略

| 数据类型 | SQLite 存储 | IndexedDB 存储 | 淘汰策略 |
| :--- | :--- | :--- | :--- |
| 聊天消息 | 全部历史 | 近 3 天 | 自动淘汰过期热数据 |
| 技能调用 | 全部历史 | 近 3 天 | 自动淘汰过期热数据 |
| 用户配置 | - | localStorage | 永久 |
| 会话状态 | - | sessionStorage | 当前会话清除 |
...
为支持混合存储模式，SQLite 数据库需要创建以下索引：

**聊天消息表索引（支持时间范围查询）：**

```sql
-- 智能体ID + 发送时间复合索引（核心索引）
CREATE INDEX IF NOT EXISTS idx_chat_messages_agent_send_time ON chat_messages(agent_id, send_time DESC);

-- 用户ID + 发送时间复合索引
CREATE INDEX IF NOT EXISTS idx_chat_messages_user_send_time ON chat_messages(user_id, send_time DESC);

-- 技能激活 + 发送时间复合索引（用于技能调用统计）
CREATE INDEX IF NOT EXISTS idx_chat_messages_skill_time ON chat_messages(skill_activated, send_time DESC);
```
...
```sql
-- 消息ID + 调用时间复合索引
CREATE INDEX IF NOT EXISTS idx_skill_calls_message_time ON skill_calls(message_id, call_time DESC);

-- 智能体ID + 状态 + 调用时间复合索引
CREATE INDEX IF NOT EXISTS idx_skill_calls_agent_status_time ON skill_calls(agent_id, status, call_time DESC);
```
...
- 每天首次启动时，将 SQLite 中最近 3 天的数据同步到 IndexedDB
- 使用后台任务批量同步，避免阻塞主线程
...
app.post('/api/sync', async (req, res) => {
    ...
    try {
        ...
        const result = stmt.run(Object.keys(data), Object.values(data));
...
// GET /api/messages/:agentId - 获取聊天消息
app.get('/api/messages/:agentId', async (req, res) => {
    ...
    try {
        const db = getDatabase();
        let query = 'SELECT * FROM chat_messages WHERE agent_id = ?';
        ...
        query += ' ORDER BY send_time DESC LIMIT 100';
...
- [IndexedDB MDN文档](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API)
- [idb库文档](https://github.com/jakearchibald/idb)
