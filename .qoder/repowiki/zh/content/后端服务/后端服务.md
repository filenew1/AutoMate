...
## 更新摘要
**所做更改**
- 更新了技能执行服务的安全性章节，反映移除shell选项的优化
- 新增了Python技能执行安全最佳实践章节
- 更新了安全策略部分，强调进程隔离和参数化执行
- 新增了命令注入防护的具体实现指导
...
**图表来源**
- [backend/index.js](file://backend/index.js#L1-L116)
...
**更新** 移除了 Python 技能执行中的 shell 选项以提高安全性
...
- 命令注入防护：使用参数化执行而非 shell 模式，避免命令注入风险。
- 环境变量控制：通过 PYTHONIOENCODING 等环境变量确保正确的字符编码。
...
- [docs/非功能设计/安全设计.md](file://docs/非功能设计/安全设计.md#L256-L295)

### Python 技能执行安全最佳实践

**新增** 针对 Python 技能执行的安全优化指导

- 移除 shell 选项：避免使用 shell: true，防止命令注入攻击
- 参数化执行：将技能路径和参数作为数组传递给 spawn
- 环境隔离：设置专用的工作目录和环境变量
- 输入验证：对传入的参数进行严格的类型检查和长度限制
- 输出控制：限制最大输出大小，防止内存溢出
- 超时机制：为子进程执行设置合理的超时时间
- 错误处理：捕获并记录详细的错误信息，便于安全审计
...
- [backend/index.js](file://backend/index.js#L32-L35)
