...
- [skills/weather_query/SKILL.md](file://skills/weather_query/SKILL.md)
...
## 更新摘要
**变更内容**
- 新增天气查询技能触发词支持，包括中文天气相关词汇如"天气"、"气温"、"温度"等
- 扩展技能关键字匹配机制，支持更丰富的中文天气查询场景
- 增强聊天界面的自然语言处理能力，提升用户体验
...
**更新** 本版本新增了天气查询触发词支持，增强了聊天界面的自然语言理解和技能激活能力。
...
- **技能关键字匹配与前置执行**：根据用户输入匹配技能，先行调用技能并把结果注入 AI 上下文。**新增天气查询触发词支持**，包括"天气"、"气温"、"温度"、"下雨"、"刮风"、"湿度"、"气象"等中文词汇。
...
### 天气查询技能
**新增功能** AutoMate 现已集成天气查询技能，支持中文天气相关词汇的自然语言处理。

#### 技能特性
- **中文触发词支持**：支持"天气"、"气温"、"温度"、"下雨"、"刮风"、"湿度"、"气象"等中文词汇
- **城市识别**：自动识别中文城市名称如"深圳"、"北京"、"上海"、"广州"等
- **多语言支持**：支持中英文混合查询，如"深圳天气"、"Beijing weather"
- **天气数据获取**：通过 OpenWeatherMap API 获取实时天气信息
- **格式化输出**：提供美观的天气报告格式，包含温度、湿度、风速等详细信息

#### 触发词映射
```typescript
'weather_query': [
  '天气', '气温', '温度', '下雨', '刮风', '湿度', '气象', 
  'weather', 'temperature', '深圳天气', '北京天气'
]
```
...
- [test_weather.py](file://test_weather.py#L1-L28)
...
- **技能匹配优化**：新增的天气查询触发词匹配采用高效的字符串查找算法，支持大小写不敏感和中文字符匹配。
...
- **天气查询失败**：检查 OpenWeatherMap API 密钥有效性，确认网络连接正常，验证城市名称是否在支持列表中。
...
## 结论
AutoMate 聊天交互模块通过清晰的组件边界、完善的流式渲染与本地存储策略、以及统一的错误处理与主题适配，实现了稳定且可扩展的聊天体验。借助技能前置执行与系统提示构建，系统具备良好的上下文感知能力。

**更新** 新增的天气查询触发词支持进一步增强了系统的自然语言处理能力，使用户能够以更自然的方式表达天气查询需求。通过中文触发词匹配和智能城市识别，系统能够准确理解用户的意图并提供相应的天气信息。

后续可在消息类型扩展、多模态内容渲染、离线同步与更细粒度的性能优化方面持续演进。
...
- **天气查询技能**：通过 OpenWeatherMap API 获取天气数据并格式化输出。支持中文触发词如"天气"、"气温"、"温度"等，自动识别中文城市名称并提供详细的天气信息。

**更新** 新增天气查询技能的详细说明，包括触发词支持和数据格式。
...
### 技能关键字匹配机制
**新增功能** ChatContainer 现已支持更丰富的技能关键字匹配，特别是天气查询技能的中文触发词支持。

#### 匹配策略
1. **精确匹配**：检查用户输入是否包含技能的触发词
2. **大小写不敏感**：支持大写和小写输入
3. **中文字符处理**：移除特殊字符后进行匹配
4. **技能名称匹配**：当触发词未匹配时，尝试匹配技能名称

#### 支持的触发词
- **天气相关**：天气、气温、温度、下雨、刮风、湿度、气象
- **英文支持**：weather、temperature、humidity、wind
- **城市查询**：各主要城市的天气查询，如"深圳天气"、"北京天气"

**章节来源**
- [src/components/chat/ChatContainer.tsx](file://src/components/chat/ChatContainer.tsx#L117-L172)