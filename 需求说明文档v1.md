...
##### 表结构说明
智能体表用于存储所有智能体的基本信息，包括智能体的名称、描述、状态、头像类型等核心属性。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 主键 | 外键 | 索引 | 备注 |
|--------|---------|---------|--------|------|------|------|------|
| id | TEXT | 是 | - | 是 | 否 | 是 | 智能体唯一标识符，使用UUID格式 |
| name | TEXT | 是 | - | 否 | 否 | 是 | 智能体名称，最大长度100字符 |
| description | TEXT | 否 | NULL | 否 | 否 | 是 | 智能体功能描述，最大长度500字符 |
| avatar_type | TEXT | 是 | 'blue' | 否 | 否 | 否 | 头像类型，可选值：blue、purple、orange、green、red、yellow |
| status | TEXT | 是 | 'offline' | 否 | 否 | 是 | 智能体状态，可选值：online、offline、busy |
| group_id | TEXT | 是 | - | 否 | 是 | 是 | 所属分组ID，关联agent_groups表 |
| icon_svg | TEXT | 是 | '' | 否 | 否 | 否 | 智能体图标SVG内容，用于渲染图标 |
| created_at | TEXT | 是 | 当前时间 | 否 | 否 | 是 | 创建时间，ISO8601格式 |
| updated_at | TEXT | 是 | 当前时间 | 否 | 否 | 是 | 最后更新时间，ISO8601格式 |
| is_deleted | INTEGER | 是 | 0 | 否 | 否 | 是 | 软删除标记，0表示未删除，1表示已删除 |
| sort_order | INTEGER | 是 | 0 | 否 | 否 | 是 | 排序字段，数字越小越靠前 |
...
-- 复合索引：分组ID + 排序字段
CREATE INDEX IF NOT EXISTS idx_agents_group_sort ON agents(group_id, sort_order);

-- 复合索引：分组ID + 状态
CREATE INDEX IF NOT EXISTS idx_agents_group_status ON agents(group_id, status);

-- 复合索引：软删除标记 + 分组ID
CREATE INDEX IF NOT EXISTS idx_agents_deleted_group ON agents(is_deleted, group_id);
```
...
FOR EACH ROW
BEGIN
    UPDATE agents SET updated_at = datetime('now', 'localtime') WHERE id = NEW.id;
END;
```
...
```sql
-- 通用助手分组下的智能体
INSERT INTO agents (id, name, description, avatar_type, status, group_id, icon_svg, sort_order) VALUES
('agent-001', '智能助手', '通用智能助手，可以帮助您处理各种任务', 'blue', 'online', 'group-general',
 '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>', 1);

-- OA办公系统分组下的智能体
INSERT INTO agents (id, name, description, avatar_type, status, group_id, icon_svg, sort_order) VALUES
('agent-002', '代码助手', '代码生成和调试助手', 'purple', 'online', 'group-oa',
 '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/>', 1),
('agent-003', '待办助手', '待办事项管理助手', 'orange', 'online', 'group-oa',
 '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>', 2);
```
...
CREATE TABLE IF NOT EXISTS agent_groups (
    ...
    updated_at TEXT NOT NULL DEFAULT (datetime('now', 'localtime')),
    is_deleted INTEGER NOT NULL DEFAULT 0,
    FOREIGN KEY (parent_id) REFERENCES agent_groups(id) ON DELETE CASCADE
);
```
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 主键 | 外键 | 索引 | 备注 |
|--------|---------|---------|--------|------|------|------|------|
| id | TEXT | 是 | - | 是 | 否 | 是 | 分组唯一标识符，使用UUID格式 |
| name | TEXT | 是 | - | 否 | 否 | 是 | 分组名称，最大长度50字符 |
| parent_id | TEXT | 否 | NULL | 否 | 是 | 是 | 父分组ID，NULL表示顶级分组 |
| icon_svg | TEXT | 是 | '' | 否 | 否 | 否 | 分组图标SVG内容 |
| is_system | INTEGER | 是 | 0 | 否 | 否 | 是 | 系统分组标记，1表示系统分组不可删除 |
| sort_order | INTEGER | 是 | 0 | 否 | 否 | 是 | 排序字段，数字越小越靠前 |
| created_at | TEXT | 是 | 当前时间 | 否 | 否 | 否 | 创建时间，ISO8601格式 |
| updated_at | TEXT | 是 | 当前时间 | 否 | 否 | 否 | 最后更新时间，ISO8601格式 |
| is_deleted | INTEGER | 是 | 0 | 否 | 否 | 是 | 软删除标记，0表示未删除，1表示已删除 |
...
-- 分组系统标记索引
CREATE INDEX IF NOT EXISTS idx_agent_groups_is_system ON agent_groups(is_system);

-- 分组排序索引
CREATE INDEX IF NOT EXISTS idx_agent_groups_sort_order ON agent_groups(sort_order);

-- 分组软删除标记索引
CREATE INDEX IF NOT EXISTS idx_agent_groups_is_deleted ON agent_groups(is_deleted);

-- 复合索引：父分组ID + 排序字段
CREATE INDEX IF NOT EXISTS idx_agent_groups_parent_sort ON agent_groups(parent_id, sort_order);

-- 复合索引：系统标记 + 软删除标记
CREATE INDEX IF NOT EXISTS idx_agent_groups_system_deleted ON agent_groups(is_system, is_deleted);
```
...
```sql
-- 更新时间触发器
CREATE TRIGGER IF NOT EXISTS trigger_agent_groups_updated_at
AFTER UPDATE ON agent_groups
FOR EACH ROW
BEGIN
    UPDATE agent_groups SET updated_at = datetime('now', 'localtime') WHERE id = NEW.id;
END;
```
...
```sql
INSERT INTO agent_groups (id, name, parent_id, icon_svg, is_system, sort_order) VALUES
('group-general', '通用助手', NULL, '', 1, 1),
('group-oa', 'OA办公系统', NULL, '', 1, 2);
```
...
##### 表结构说明
用户偏好表用于存储用户的界面偏好设置，包括主题、分组展开状态、侧边栏状态等。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 主键 | 外键 | 索引 | 备注 |
|--------|---------|---------|--------|------|------|------|------|
| id | TEXT | 是 | - | 是 | 否 | 是 | 偏好设置唯一标识符，使用UUID格式 |
| user_id | TEXT | 是 | - | 否 | 否 | 是 | 用户唯一标识符，单用户系统固定为'user-001' |
| preference_key | TEXT | 是 | - | 否 | 否 | 是 | 偏好设置键名，如'theme'、'group_states'等 |
| preference_value | TEXT | 是 | - | 否 | 否 | 否 | 偏好设置值，JSON字符串格式存储复杂类型 |
| created_at | TEXT | 是 | 当前时间 | 否 | 否 | 否 | 创建时间，ISO8601格式 |
| updated_at | TEXT | 是 | 当前时间 | 否 | 否 | 否 | 最后更新时间，ISO8601格式 |

##### 偏好键说明

| preference_key | preference_value格式 | 说明 |
|----------------|---------------------|------|
| theme | 'light' 或 'dark' | 用户主题偏好 |
| group_states | JSON对象，键为分组ID，值为'expanded'或'collapsed' | 各分组展开/折叠状态 |
| sidebar_state | 'expanded' 或 'collapsed' | 侧边栏展开/折叠状态 |
| selected_agent_id | 智能体ID | 当前选中的智能体ID |
| font_size | 'small'、'medium'、'large' | 字体大小偏好 |
...
FOR EACH ROW
BEGIN
    UPDATE user_preferences SET updated_at = datetime('now', 'localtime') WHERE id = NEW.id;
END;
```
...
('pref-theme', 'user-001', 'theme', 'light'),
('pref-sidebar', 'user-001', 'sidebar_state', 'expanded'),
('pref-group-states', 'user-001', 'group_states', '{"group-general":"expanded","group-oa":"expanded"}');
```
...
##### 表结构说明
搜索历史表用于存储用户的搜索关键词历史记录，最多保留10条记录。
...
CREATE TABLE IF NOT EXISTS search_history (
    ...
    search_time TEXT NOT NULL DEFAULT (datetime('now', 'localtime'))
);
```
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 主键 | 外键 | 索引 | 备注 |
|--------|---------|---------|--------|------|------|------|------|
| id | TEXT | 是 | - | 是 | 否 | 是 | 搜索历史唯一标识符，使用UUID格式 |
| user_id | TEXT | 是 | - | 否 | 否 | 是 | 用户唯一标识符 |
| keyword | TEXT | 是 | - | 否 | 否 | 是 | 搜索关键词，已过滤SQL注入字符，最大长度100字符 |
| search_time | TEXT | 是 | 当前时间 | 否 | 否 | 是 | 搜索时间，ISO8601格式，用于排序 |
...
-- 复合索引：用户ID + 搜索时间
CREATE INDEX IF NOT EXISTS idx_search_history_user_time ON search_history(user_id, search_time DESC);
```
...
##### 表结构说明
系统配置表用于存储系统级别的配置信息，如版本号、默认主题、功能开关等。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 主键 | 外键 | 索引 | 备注 |
|--------|---------|---------|--------|------|------|------|------|
| id | TEXT | 是 | - | 是 | 否 | 是 | 配置项唯一标识符，使用UUID格式 |
| config_key | TEXT | 是 | - | 否 | 否 | 是 | 配置键名，全局唯一，如'app_version'、'default_theme'等 |
| config_value | TEXT | 是 | - | 否 | 否 | 否 | 配置值，根据config_type决定解析方式 |
| config_type | TEXT | 是 | 'string' | 否 | 否 | 否 | 配置值类型，可选值：string、number、boolean、json |
| description | TEXT | 否 | NULL | 否 | 否 | 否 | 配置项说明 |
| created_at | TEXT | 是 | 当前时间 | 否 | 否 | 否 | 创建时间，ISO8601格式 |
| updated_at | TEXT | 是 | 当前时间 | 否 | 否 | 否 | 最后更新时间，ISO8601格式 |
...
| max_search_history | '10' | number | 最大搜索历史记录数 |
| enable_dark_mode | 'true' | boolean | 是否启用深色模式 |
| sidebar_default_width | '320' | number | 侧边栏默认宽度（像素） |
| sidebar_min_width | '60' | number | 侧边栏最小宽度（像素） |
| animation_enabled | 'true' | boolean | 是否启用动画效果 |
| message_load_batch_size | '50' | number | 消息加载批次大小 |
| max_message_display | '1000' | number | 最大显示消息数 |
...
('conf-001', 'app_version', '1.0.0', 'string', '应用版本号'),
('conf-002', 'default_theme', 'light', 'string', '默认主题'),
('conf-003', 'max_search_history', '10', 'number', '最大搜索历史记录数'),
('conf-004', 'enable_dark_mode', 'true', 'boolean', '是否启用深色模式'),
('conf-005', 'sidebar_default_width', '320', 'number', '侧边栏默认宽度（像素）'),
('conf-006', 'sidebar_min_width', '60', 'number', '侧边栏最小宽度（像素）'),
('conf-007', 'animation_enabled', 'true', 'boolean', '是否启用动画效果'),
('conf-008', 'message_load_batch_size', '50', 'number', '消息加载批次大小'),
('conf-009', 'max_message_display', '1000', 'number', '最大显示消息数');
```
...
所有表的外键关系已通过FOREIGN KEY约束定义，确保数据完整性：

- `agents.group_id` → `agent_groups.id`：智能体所属分组
- `agent_groups.parent_id` → `agent_groups.id`：分组嵌套关系

##### 级联删除规则

- 当删除分组时，该分组下的所有智能体会被级联删除
- 当删除父分组时，其子分组会被级联删除

##### 软删除策略

- `agents`表和`agent_groups`表使用`is_deleted`字段实现软删除
- 所有查询默认过滤`is_deleted = 1`的记录
- 软删除记录在数据库中保留30天后可定期清理
...
```sql
-- 清理30天前的软删除记录
DELETE FROM agents WHERE is_deleted = 1 AND datetime(updated_at) < datetime('now', '-30 days');
DELETE FROM agent_groups WHERE is_deleted = 1 AND datetime(updated_at) < datetime('now', '-30 days');
```
...
- 定期执行`VACUUM`命令优化数据库文件大小
- 定期执行`ANALYZE`命令更新查询优化器统计信息
- 监控查询性能，必要时调整索引策略
...
| 字段名 | 数据类型 | 约束 | 描述 |
| :--- | :--- | :--- | :--- |
| `agent_id` | `TEXT` | `PRIMARY KEY` | 智能体唯一标识 |
| `agent_name` | `TEXT` | `NOT NULL` | 智能体名称 |
| `avatar_type` | `TEXT` | `NOT NULL` | 智能体头像类型 |
| `status` | `TEXT` | `NOT NULL DEFAULT 'offline'` | 智能体状态（online/offline） |
...
| `agent_id` | `TEXT` | `NOT NULL REFERENCES agents(agent_id)` | 智能体ID |
| `user_id` | `TEXT` | `NOT NULL REFERENCES users(user_id)` | 用户ID |
| `last_message_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | 最后消息时间 |
| `message_count` | `INTEGER` | `DEFAULT 0` | 消息数量 |
| `status` | `TEXT` | `NOT NULL DEFAULT 'active'` | 对话状态（active/archived） |
| `created_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | 创建时间 |
| `updated_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | 更新时间 |
...
| `conversation_id` | `TEXT` | `NOT NULL REFERENCES conversations(conversation_id)` | 对话ID |
| `sender_type` | `TEXT` | `NOT NULL` | 发送者类型（user/agent） |
| `sender_id` | `TEXT` | `NOT NULL` | 发送者ID |
| `content` | `TEXT` | `NOT NULL` | 消息内容 |
| `status` | `TEXT` | `NOT NULL DEFAULT 'sent'` | 消息状态（sending/sent/failed） |
...
| 字段名 | 数据类型 | 约束 | 描述 |
| :--- | :--- | :--- | :--- |
| `attachment_id` | `TEXT` | `PRIMARY KEY` | 附件唯一标识 |
| `message_id` | `TEXT` | `NOT NULL REFERENCES messages(message_id)` | 消息ID |
| `file_name` | `TEXT` | `NOT NULL` | 文件名 |
| `file_path` | `TEXT` | `NOT NULL` | 文件存储路径 |
| `file_size` | `INTEGER` | `NOT NULL` | 文件大小（字节） |
| `file_type` | `TEXT` | `NOT NULL` | 文件类型 |
| `upload_status` | `TEXT` | `NOT NULL DEFAULT 'uploading'` | 上传状态（uploading/success/failed） |
| `created_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | 创建时间 |
| `updated_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | 更新时间 |
...
#### 消息编辑历史表（message_edits）
| 字段名 | 数据类型 | 约束 | 描述 |
| :--- | :--- | :--- | :--- |
| `edit_id` | `TEXT` | `PRIMARY KEY` | 编辑记录唯一标识 |
| `message_id` | `TEXT` | `NOT NULL REFERENCES messages(message_id)` | 消息ID |
| `old_content` | `TEXT` | `NOT NULL` | 编辑前内容 |
| `new_content` | `TEXT` | `NOT NULL` | 编辑后内容 |
| `edited_by` | `TEXT` | `NOT NULL` | 编辑者ID |
| `created_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | 创建时间 |
...
| `status` | `TEXT` | `NOT NULL DEFAULT 'pending'` | 草稿状态（pending/sent） |
| `created_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | 创建时间 |
| `updated_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | 更新时间 |

**索引**：
- `CREATE INDEX idx_drafts_user_agent ON drafts(user_id, agent_id);`
...
| 字段名 | 数据类型 | 约束 | 描述 |
| :--- | :--- | :--- | :--- |
| `paste_id` | `TEXT` | `PRIMARY KEY` | 粘贴记录唯一标识 |
| `user_id` | `TEXT` | `NOT NULL REFERENCES users(user_id)` | 用户ID |
| `content` | `TEXT` | `NOT NULL` | 粘贴内容 |
| `content_type` | `TEXT` | `NOT NULL DEFAULT 'text'` | 内容类型（text/image） |
| `created_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | 创建时间 |

**索引**：
- `CREATE INDEX idx_paste_history_user ON paste_history(user_id);`
- `CREATE INDEX idx_paste_history_created_at ON paste_history(created_at);`
...
-- 草稿表索引
CREATE INDEX IF NOT EXISTS idx_drafts_user_agent ON drafts(user_id, agent_id);
CREATE INDEX IF NOT EXISTS idx_drafts_status ON drafts(status);
CREATE INDEX IF NOT EXISTS idx_drafts_updated_at ON drafts(updated_at);

-- 粘贴历史表索引
CREATE INDEX IF NOT EXISTS idx_paste_history_user ON paste_history(user_id);
CREATE INDEX IF NOT EXISTS idx_paste_history_created_at ON paste_history(created_at);
```
...
##### 表结构说明
系统统计表用于存储系统统计数据，包括可用智能体数量、可用技能数量、对话次数等，支持欢迎页面的统计卡片显示。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 主键 | 外键 | 索引 | 备注 |
|--------|---------|---------|--------|------|------|------|------|
| id | TEXT | 是 | - | 是 | 否 | 是 | 统计项唯一标识符，使用UUID格式 |
| stat_type | TEXT | 是 | - | 否 | 否 | 是 | 统计类型，如：available_agents、available_skills、conversation_count等 |
| value | INTEGER | 是 | - | 否 | 否 | 是 | 统计值 |
| description | TEXT | 否 | NULL | 否 | 否 | 否 | 统计项描述 |
| last_updated | TEXT | 是 | 当前时间 | 否 | 否 | 是 | 最后更新时间，ISO8601格式 |
| is_active | INTEGER | 是 | 1 | 否 | 否 | 是 | 是否激活，1表示激活，0表示未激活 |
...
-- 统计值索引
CREATE INDEX IF NOT EXISTS idx_system_stats_value ON system_stats(value);

-- 最后更新时间索引
CREATE INDEX IF NOT EXISTS idx_system_stats_updated ON system_stats(last_updated);

-- 激活状态索引
CREATE INDEX IF NOT EXISTS idx_system_stats_active ON system_stats(is_active);

-- 复合索引：类型 + 激活状态
CREATE INDEX IF NOT EXISTS idx_system_stats_type_active ON system_stats(stat_type, is_active);
```
...
```sql
-- 更新时间触发器
CREATE TRIGGER IF NOT EXISTS trigger_system_stats_updated_at
AFTER UPDATE ON system_stats
FOR EACH ROW
BEGIN
    UPDATE system_stats SET last_updated = datetime('now', 'localtime') WHERE id = NEW.id;
END;
```
...
```sql
-- 系统统计初始化数据
INSERT INTO system_stats (id, stat_type, value, description) VALUES
('stat-001', 'available_agents', 3, '可用智能体数量'),
('stat-002', 'available_skills', 9, '可用技能数量'),
('stat-003', 'conversation_count', 0, '对话次数（实际显示为无限符号）');
```

#### 2.3.5.2 用户偏好设置表

##### 表结构说明
用户偏好设置表用于存储用户的主题偏好、界面配置等个性化设置，支持欢迎页面的主题适配功能。
...
|--------|---------|---------|--------|------|------|------|------|
| id | TEXT | 是 | - | 是 | 否 | 是 | 偏好设置唯一标识符，使用UUID格式 |
| preference_key | TEXT | 是 | - | 否 | 否 | 是 | 偏好设置键，如：theme、welcome_page_visible等 |
| preference_value | TEXT | 是 | - | 否 | 否 | 否 | 偏好设置值，如：light、dark、true、false等 |
| description | TEXT | 否 | NULL | 否 | 否 | 否 | 偏好设置描述 |
| created_at | TEXT | 是 | 当前时间 | 否 | 否 | 是 | 创建时间，ISO8601格式 |
| updated_at | TEXT | 是 | 当前时间 | 否 | 否 | 是 | 最后更新时间，ISO8601格式 |
...
-- 创建时间索引
CREATE INDEX IF NOT EXISTS idx_user_preferences_created ON user_preferences(created_at);
...
```sql
-- 用户偏好设置初始化数据
INSERT INTO user_preferences (id, preference_key, preference_value, description) VALUES
('pref-001', 'theme', 'light', '界面主题：light（浅色）或 dark（深色）'),
('pref-002', 'welcome_page_visible', 'true', '欢迎页面是否可见'),
('pref-003', 'welcome_page_animation', 'true', '欢迎页面是否启用动画效果'),
('pref-004', 'welcome_page_layout', 'centered', '欢迎页面布局方式');
```

#### 2.3.5.3 欢迎页面配置表

##### 表结构说明
欢迎页面配置表用于存储欢迎页面的具体配置信息，包括布局设置、显示选项、动画效果等。

##### 表定义

```sql
CREATE TABLE IF NOT EXISTS welcome_page_config (
    id TEXT PRIMARY KEY NOT NULL,
...
|--------|---------|---------|--------|------|------|------|------|
| id | TEXT | 是 | - | 是 | 否 | 是 | 配置项唯一标识符，使用UUID格式 |
| config_key | TEXT | 是 | - | 否 | 否 | 是 | 配置键，如：layout_type、show_icons、animation_duration等 |
| config_value | TEXT | 是 | - | 否 | 否 | 否 | 配置值 |
| config_type | TEXT | 是 | 'string' | 否 | 否 | 是 | 配置类型，如：string、number、boolean等 |
| description | TEXT | 否 | NULL | 否 | 否 | 否 | 配置项描述 |
| last_updated | TEXT | 是 | 当前时间 | 否 | 否 | 是 | 最后更新时间，ISO8601格式 |
| is_default | INTEGER | 是 | 0 | 否 | 否 | 是 | 是否默认配置，1表示默认，0表示非默认 |

##### 索引设计

```sql
-- 配置键索引
CREATE INDEX IF NOT EXISTS idx_welcome_config_key ON welcome_page_config(config_key);

-- 配置类型索引
CREATE INDEX IF NOT EXISTS idx_welcome_config_type ON welcome_page_config(config_type);

-- 最后更新时间索引
CREATE INDEX IF NOT EXISTS idx_welcome_config_updated ON welcome_page_config(last_updated);

-- 默认配置索引
CREATE INDEX IF NOT EXISTS idx_welcome_config_default ON welcome_page_config(is_default);

-- 复合索引：配置键 + 默认状态
CREATE INDEX IF NOT EXISTS idx_welcome_config_key_default ON welcome_page_config(config_key, is_default);
```
...
```sql
-- 更新时间触发器
CREATE TRIGGER IF NOT EXISTS trigger_welcome_config_updated_at
AFTER UPDATE ON welcome_page_config
FOR EACH ROW
BEGIN
    UPDATE welcome_page_config SET last_updated = datetime('now', 'localtime') WHERE id = NEW.id;
END;
```
...
```sql
-- 欢迎页面配置初始化数据
INSERT INTO welcome_page_config (id, config_key, config_value, config_type, description, is_default) VALUES
('config-001', 'layout_type', 'centered', 'string', '页面布局类型：centered（居中）、fullscreen（全屏）等', 1),
('config-002', 'show_icons', 'true', 'boolean', '是否显示欢迎图标', 1),
('config-003', 'show_stat_cards', 'true', 'boolean', '是否显示统计卡片', 1),
('config-004', 'animation_duration', '300', 'number', '动画持续时间（毫秒）', 1),
('config-005', 'card_layout', 'grid', 'string', '统计卡片布局：grid（网格）、list（列表）', 1),
('config-006', 'card_count', '3', 'number', '统计卡片数量', 1),
('config-007', 'responsive_breakpoint', '768', 'number', '响应式断点（像素）', 1);
```
...
1. **定期更新统计数据**：
   - 建议每5分钟更新一次系统统计数据，确保欢迎页面显示的统计信息准确
   - 可通过定时任务或触发事件更新统计数据

2. **索引优化**：
   - 对于频繁查询的统计类型和配置项，建议创建复合索引
   - 定期重建索引以提高查询性能

3. **数据清理**：
   - 系统统计表中的历史数据可定期清理，只保留最新的统计记录
   - 建议保留最近30天的历史统计数据

4. **性能监控**：
   - 监控数据库查询性能，特别是统计数据查询
   - 对于大型系统，考虑使用缓存机制减少数据库查询

5. **备份策略**：
   - 定期备份用户偏好设置和欢迎页面配置
   - 建议使用增量备份策略，减少备份时间和空间占用
...
#### 2.4.5.1 系统配置表（system_config）

| 字段名 | 数据类型 | 长度 | 主键 | 外键 | 索引 | 允许空 | 默认值 | 备注 |
|--------|----------|------|------|------|------|--------|--------|------|
| id | INTEGER | - | 是 | 否 | 是 | 否 | AUTOINCREMENT | 配置ID |
| config_key | TEXT | 255 | 否 | 否 | 是 | 否 | - | 配置键名 |
| config_value | TEXT | 1024 | 否 | 否 | 否 | 否 | - | 配置值 |
| config_type | TEXT | 50 | 否 | 否 | 是 | 否 | 'system' | 配置类型 |
| description | TEXT | 512 | 否 | 否 | 否 | 是 | NULL | 配置描述 |
| created_at | TIMESTAMP | - | 否 | 否 | 否 | 否 | CURRENT_TIMESTAMP | 创建时间 |
| updated_at | TIMESTAMP | - | 否 | 否 | 否 | 否 | CURRENT_TIMESTAMP | 更新时间 |

**索引**：
- 主键索引：`CREATE UNIQUE INDEX idx_system_config_id ON system_config(id);`
- 配置键索引：`CREATE UNIQUE INDEX idx_system_config_key ON system_config(config_key);`
- 配置类型索引：`CREATE INDEX idx_system_config_type ON system_config(config_type);`

**备注**：
- 存储系统级配置信息，如主题设置、侧边栏状态等
- config_key示例：'theme', 'sidebar_collapsed', 'auto_update'
- config_value示例：'light', 'true', 'false'

#### 2.4.5.2 应用版本表（app_version）

| 字段名 | 数据类型 | 长度 | 主键 | 外键 | 索引 | 允许空 | 默认值 | 备注 |
|--------|----------|------|------|------|------|--------|--------|------|
| id | INTEGER | - | 是 | 否 | 是 | 否 | AUTOINCREMENT | 版本ID |
| version_code | TEXT | 50 | 否 | 否 | 是 | 否 | - | 版本号 |
| version_name | TEXT | 255 | 否 | 否 | 否 | 是 | NULL | 版本名称 |
| release_date | DATE | - | 否 | 否 | 否 | 是 | NULL | 发布日期 |
| is_current | BOOLEAN | - | 否 | 否 | 是 | 否 | 0 | 是否当前版本 |
| description | TEXT | 1024 | 否 | 否 | 否 | 是 | NULL | 版本描述 |
| created_at | TIMESTAMP | - | 否 | 否 | 否 | 否 | CURRENT_TIMESTAMP | 创建时间 |

**索引**：
- 主键索引：`CREATE UNIQUE INDEX idx_app_version_id ON app_version(id);`
- 版本号索引：`CREATE UNIQUE INDEX idx_app_version_code ON app_version(version_code);`
- 当前版本索引：`CREATE INDEX idx_app_version_current ON app_version(is_current);`

**备注**：
- 存储应用版本信息，用于显示版本号
- 每次发布新版本时，将旧版本的is_current设为0，新版本设为1

#### 2.4.5.3 系统状态表（system_status）

| 字段名 | 数据类型 | 长度 | 主键 | 外键 | 索引 | 允许空 | 默认值 | 备注 |
|--------|----------|------|------|------|------|--------|--------|------|
| id | INTEGER | - | 是 | 否 | 是 | 否 | AUTOINCREMENT | 状态ID |
| status_key | TEXT | 255 | 否 | 否 | 是 | 否 | - | 状态键名 |
| status_value | TEXT | 512 | 否 | 否 | 否 | 否 | - | 状态值 |
| status_type | TEXT | 50 | 否 | 否 | 是 | 否 | 'system' | 状态类型 |
| last_updated | TIMESTAMP | - | 否 | 否 | 否 | 否 | CURRENT_TIMESTAMP | 最后更新时间 |

**索引**：
- 主键索引：`CREATE UNIQUE INDEX idx_system_status_id ON system_status(id);`
- 状态键索引：`CREATE UNIQUE INDEX idx_system_status_key ON system_status(status_key);`
- 状态类型索引：`CREATE INDEX idx_system_status_type ON system_status(status_type);`

**备注**：
- 存储系统状态信息，如在线状态等
- status_key示例：'online_status', 'last_sync_time'
- status_value示例：'online', '2024-01-01 12:00:00'
...
CREATE TABLE IF NOT EXISTS system_status (
    ...
    status_type TEXT NOT NULL DEFAULT 'system',
...
```sql
-- 系统配置表索引
CREATE UNIQUE INDEX IF NOT EXISTS idx_system_config_id ON system_config(id);
...
-- 应用版本表索引
CREATE UNIQUE INDEX IF NOT EXISTS idx_app_version_id ON app_version(id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_app_version_code ON app_version(version_code);
CREATE INDEX IF NOT EXISTS idx_app_version_current ON app_version(is_current);

-- 系统状态表索引
CREATE UNIQUE INDEX IF NOT EXISTS idx_system_status_id ON system_status(id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_system_status_key ON system_status(status_key);
CREATE INDEX IF NOT EXISTS idx_system_status_type ON system_status(status_type);
```
...
('theme', 'light', 'system', '系统主题（light/dark）'),
('sidebar_collapsed', 'false', 'system', '侧边栏是否收缩'),
('auto_update', 'true', 'system', '是否自动更新'),
('language', 'zh-CN', 'system', '系统语言');

-- 初始化应用版本数据
INSERT OR IGNORE INTO app_version (version_code, version_name, release_date, is_current, description) VALUES
('1.0.0', '初始版本', DATE('now'), 1, '系统设置模块初始版本');

-- 初始化系统状态数据
INSERT OR IGNORE INTO system_status (status_key, status_value, status_type) VALUES
('online_status', 'online', 'system'),
('last_sync_time', CURRENT_TIMESTAMP, 'system');
```
...
-- 系统状态表更新时间触发器
CREATE TRIGGER IF NOT EXISTS trg_system_status_updated_at
AFTER UPDATE ON system_status
BEGIN
    UPDATE system_status SET last_updated = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;

-- 应用版本表当前版本触发器
CREATE TRIGGER IF NOT EXISTS trg_app_version_current
AFTER INSERT ON app_version
FOR EACH ROW
BEGIN
    UPDATE app_version SET is_current = 0 WHERE id != NEW.id;
    UPDATE app_version SET is_current = 1 WHERE id = NEW.id;
END;
```
...
| 字段名 | 数据类型 | 长度 | 约束 | 描述 |
| :--- | :--- | :--- | :--- | :--- |
| `theme_id` | `INTEGER` | - | `PRIMARY KEY AUTOINCREMENT` | 主题ID |
| `theme_key` | `VARCHAR` | `50` | `UNIQUE NOT NULL` | 主题标识（如'light'、'dark'） |
| `theme_name` | `VARCHAR` | `100` | `NOT NULL` | 主题名称 |
| `is_default` | `BOOLEAN` | - | `DEFAULT 0` | 是否为默认主题 |
| `created_at` | `TIMESTAMP` | - | `DEFAULT CURRENT_TIMESTAMP` | 创建时间 |
| `updated_at` | `TIMESTAMP` | - | `DEFAULT CURRENT_TIMESTAMP` | 更新时间 |

**索引**：
- `idx_theme_key`：在`theme_key`字段上创建唯一索引，加速主题标识的查找

##### 1.2 theme_settings表（主题样式设置）
| 字段名 | 数据类型 | 长度 | 约束 | 描述 |
| :--- | :--- | :--- | :--- | :--- |
| `setting_id` | `INTEGER` | - | `PRIMARY KEY AUTOINCREMENT` | 设置ID |
| `theme_id` | `INTEGER` | - | `NOT NULL REFERENCES themes(theme_id)` | 主题ID（外键） |
| `setting_key` | `VARCHAR` | `100` | `NOT NULL` | 设置键（如'body_background'） |
| `setting_value` | `VARCHAR` | `255` | `NOT NULL` | 设置值（如'bg-gray-100'） |
| `created_at` | `TIMESTAMP` | - | `DEFAULT CURRENT_TIMESTAMP` | 创建时间 |
| `updated_at` | `TIMESTAMP` | - | `DEFAULT CURRENT_TIMESTAMP` | 更新时间 |

**索引**：
- `idx_theme_setting`：在`theme_id`和`setting_key`字段上创建复合索引，加速主题设置的查找

##### 1.3 user_theme_preferences表（用户主题偏好）
| 字段名 | 数据类型 | 长度 | 约束 | 描述 |
| :--- | :--- | :--- | :--- | :--- |
| `preference_id` | `INTEGER` | - | `PRIMARY KEY AUTOINCREMENT` | 偏好ID |
| `user_id` | `INTEGER` | - | `DEFAULT 1` | 用户ID（默认1，适用于单用户场景） |
| `theme_id` | `INTEGER` | - | `NOT NULL REFERENCES themes(theme_id)` | 主题ID（外键） |
| `is_active` | `BOOLEAN` | - | `DEFAULT 1` | 是否为当前激活的主题 |
| `created_at` | `TIMESTAMP` | - | `DEFAULT CURRENT_TIMESTAMP` | 创建时间 |
| `updated_at` | `TIMESTAMP` | - | `DEFAULT CURRENT_TIMESTAMP` | 更新时间 |

**索引**：
- `idx_user_theme`：在`user_id`和`is_active`字段上创建复合索引，加速用户当前主题的查找
...
-- 插入默认主题数据
INSERT OR IGNORE INTO themes (theme_key, theme_name, is_default) VALUES
('light', '浅色主题', 1),
('dark', '深色主题', 0);

-- 插入浅色主题设置
INSERT OR IGNORE INTO theme_settings (theme_id, setting_key, setting_value) VALUES
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'body_background', 'bg-gray-100'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'body_text', 'text-gray-900'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'sidebar_background', 'bg-white'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'sidebar_border', 'border-gray-300'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'sidebar_search_background', 'bg-gray-100'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'sidebar_search_text', 'text-gray-900'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'sidebar_group_background', 'bg-gray-100'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'sidebar_group_text', 'text-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'sidebar_item_hover', 'hover:bg-gray-200'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'sidebar_bottom_border', 'border-gray-200'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'content_background', 'bg-gray-50'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'content_header_border', 'border-gray-200'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'chat_background', 'bg-gray-50'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'chat_divider', 'bg-gray-300'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'chat_agent_message', 'bg-gray-200'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'chat_user_message', 'bg-blue-100'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'chat_input_area', 'bg-white'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'chat_input_field', 'bg-gray-100'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'chat_input_text', 'text-gray-900'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'chat_button_area', 'bg-gray-50'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'welcome_card_background', 'bg-gradient-to-br from-gray-100 to-gray-200'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'welcome_stat_text', 'text-blue-600'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'welcome_label_text', 'text-gray-600'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'welcome_desc_text', 'text-gray-600'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'bottom_settings_hover', 'hover:bg-gray-200'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'bottom_settings_text', 'text-gray-600'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'bottom_settings_icon', 'text-gray-400'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'bottom_settings_avatar_background', 'bg-gray-100'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'bottom_settings_avatar_inner', 'bg-gray-200'),
((SELECT theme_id FROM themes WHERE theme_key = 'light'), 'bottom_settings_avatar_icon', 'text-gray-600');

-- 插入深色主题设置
INSERT OR IGNORE INTO theme_settings (theme_id, setting_key, setting_value) VALUES
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'body_background', 'bg-gray-900'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'body_text', 'text-white'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'sidebar_background', 'bg-gray-800'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'sidebar_border', 'border-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'sidebar_search_background', 'bg-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'sidebar_search_text', 'text-white'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'sidebar_group_background', 'bg-gray-700/30'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'sidebar_group_text', 'text-gray-200'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'sidebar_item_hover', 'hover:bg-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'sidebar_bottom_border', 'border-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'content_background', 'bg-gray-900'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'content_header_border', 'border-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'chat_background', 'bg-gray-900'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'chat_divider', 'bg-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'chat_agent_message', 'bg-gray-800'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'chat_user_message', 'bg-blue-600'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'chat_input_area', 'bg-gray-800'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'chat_input_field', 'bg-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'chat_input_text', 'text-white'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'chat_button_area', 'bg-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'welcome_card_background', 'bg-gray-800'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'welcome_stat_text', 'text-blue-400'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'welcome_label_text', 'text-gray-400'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'welcome_desc_text', 'text-gray-400'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'bottom_settings_hover', 'hover:bg-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'bottom_settings_text', 'text-gray-300'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'bottom_settings_icon', 'text-gray-300'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'bottom_settings_avatar_background', 'bg-gray-600'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'bottom_settings_avatar_inner', 'bg-gray-700'),
((SELECT theme_id FROM themes WHERE theme_key = 'dark'), 'bottom_settings_avatar_icon', 'text-gray-300');

-- 插入默认用户主题偏好
INSERT OR IGNORE INTO user_theme_preferences (user_id, theme_id, is_active) VALUES
(1, (SELECT theme_id FROM themes WHERE is_default = 1), 1);
```
...
```sql
-- 创建themes表索引
CREATE UNIQUE INDEX IF NOT EXISTS idx_theme_key ON themes(theme_key);

-- 创建theme_settings表索引
CREATE INDEX IF NOT EXISTS idx_theme_setting ON theme_settings(theme_id, setting_key);

-- 创建user_theme_preferences表索引
CREATE INDEX IF NOT EXISTS idx_user_theme ON user_theme_preferences(user_id, is_active);
```
...
##### 4.1 主题切换流程
1. 用户点击主题切换按钮
2. 应用程序更新`user_theme_preferences`表中对应用户的`theme_id`和`is_active`字段
3. 同时更新其他主题的`is_active`为0
4. 应用新主题的样式设置

##### 4.2 主题设置更新
1. 当需要修改主题样式时，更新`theme_settings`表中对应主题的设置值
2. 应用程序重新加载主题设置并应用新样式
